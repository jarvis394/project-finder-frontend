...
const Wrapper = styled(motion.div, {
  shouldForwardProp: (prop) => prop !== 'circleVariant',
})<{ circleVariant: 'like' | 'reject' }>(({ circleVariant }) => ({
  position: 'absolute',
...
const StyledCircle = styled('div', {
  shouldForwardProp: (prop) => prop !== 'circleVariant',
})<{ circleVariant: 'like' | 'reject' }>(({ theme, circleVariant }) => ({
  width: CIRCLE_WIDTH,
  height: CIRCLE_WIDTH,
  backgroundColor:
...
const Circle: React.FC<CircleProps & MotionProps> = ({
  circleControls,
  x,
  variant,
  ...props
